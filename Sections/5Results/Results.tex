...
Figure \ref{fig:pp-pPb} shows the average of $\Dzero$, $\Dplus$ and $\Dstar$ azimuthal correlations for pp for several ranges of trigger and associated $p_\mathrm{T}$, compared to different Pythia6 tunes (Perugia 0, 2010, 2011), Pythia 8 (tune 4C) and POWHEG+PYTHIA at the same collision energy. A substantial agreement in the overall momentum evolution of the correlation pattern is observed within uncertainties. In Figure~\ref{fig:pp-models1} and ~\ref{fig:pp-models2} the comparison of the extracted physical observables(near-side and away-side yield, width and baseline height). %, though all the simulations underestimate the near side, especially for the highest D-meson $p_{t}$ range.
